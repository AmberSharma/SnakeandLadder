		...
		if(attempt <= last && (inside = 1))
		{
			...
			if(attempt == last)
			{
				for(var i = 0 ; i < snakeface.length ; i ++)
				{
					...
					{
						$("#"+last).html("");
						$("#"+snaketail[i]).html("<img src='<?php echo SITE_URL;?>/images/"+avatar+"' height='30' width='30'/>");						last = snaketail[i] ; 
						inside = 0;
						Timer4 = setTimeout('func(' + (attempt + 1) + ')', 1000);
					}
				}
				...
				for(var i = 0 ; i < laddertail.length ; i ++)
				{
					...
					{
						$("#"+(last)).html("");
						$("#"+ladderface[i]).html("<img src='<?php echo SITE_URL;?>/images/"+avatar+"' height='30' width='30'/>");						last = laddertail[i];
						inside = 0;
						Timer4 = setTimeout('func(' + (attempt + 1) + ')', 1000);
					}
				}
